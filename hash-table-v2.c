...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	hash_table->bucket_mutexes = malloc(sizeof(pthread_mutex_t) * HASH_TABLE_CAPACITY);
    for (int i = 0; i < HASH_TABLE_CAPACITY; i++) {
        pthread_mutex_init(&hash_table->bucket_mutexes[i], NULL);
    }
	return hash_table;
}
...
{
	uint32_t index = bernstein_hash(key) % HASH_TABLE_CAPACITY;
    pthread_mutex_lock(&hash_table->bucket_mutexes[index]);
	...
	if (list_entry != NULL) {
		...
		pthread_mutex_unlock(&hash_table->bucket_mutexes[index]);
		return;
	}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	...
	for (int i = 0; i < HASH_TABLE_CAPACITY; i++) {
        pthread_mutex_destroy(&hash_table->bucket_mutexes[i]);
    }
