...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		if (pthread_mutex_init(&entry->bucket_mutex, NULL) !=0)
			exit(EXIT_FAILURE);
	}
	return hash_table;
}
...
{    
	...
	if (pthread_mutex_lock(&hash_table_entry->bucket_mutex) !=0)
			exit(EXIT_FAILURE);

	/* Update the value if it already exists */
	if (list_entry != NULL) {
		list_entry->value = value;
		if (pthread_mutex_unlock(&hash_table_entry->bucket_mutex) !=0)
			exit(EXIT_FAILURE);
		return;
	}
